        ...
        where TGraph : class, IBidirectionalGraph<TVertex, TEdge>
    {
        ...
        public void HighlightTriggerEventHandler(object sender, HighlightTriggeredEventArgs args)
        {
            ...
            else if (args.OriginalSource is EdgeControl)
            {
                ...
                bool IsDeleted = false;  //CSR 4/3/14 support marking as deleted
                try
                {
                    edge = (TEdge)ec.Edge;
                    IsDeleted = (edge is IDeletableEdge && ((IDeletableEdge)edge).IsDeleted());  //CSR 4/3/14 support marking as deleted
                }
                catch
                {
                }
                ec.Deleted = IsDeleted;  //CSR 4/3/14 support marking as deleted
