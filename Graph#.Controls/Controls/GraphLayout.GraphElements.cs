...
namespace GraphSharp.Controls
{
        ...
        where TGraph : class, IBidirectionalGraph<TVertex, TEdge>
    {
        ...
        //CSR 4/14 add handling for deleted flag
        protected EdgeControl GetOrCreateEdgeControl(TEdge edge)
        {
            EdgeControl oEdgeControl;
            if (_edgeControls.TryGetValue(edge, out oEdgeControl))
            {
                oEdgeControl.Deleted = edge is IDeletableEdge && ((IDeletableEdge)edge).IsDeleted();
            }
            else
            {
                CreateEdgeControl(edge);
        ...
        protected virtual void CreateEdgeControl(TEdge edge)
        {
            ...
            edgeControl.Deleted = (edge is IDeletableEdge && ((IDeletableEdge)edge).IsDeleted());  //CSR 4/3/14 support marking an edge as deleted
